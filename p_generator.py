...
def scan_column(cur,conn):
    global mxYear, mnYear, mxRntms, mnRntms,mxRnk,mnRnk
    idxCtlg =[]
    lValue=[]
    try:
        index = 0
        for ctlg in lFtrCols:
            ...
            if ctlg in ['year','runtimes']:
                ...
                if nullflag:
                    ...
                    index += 1
            elif ctlg in ["cast_1st_rank","cast_2nd_rank","cast_3rd_rank"]:
                for instance in result:
                    instance = int(instance[0])
                    if instance > mxRnk:
                        mxRnk = instance
    ...
    try:
        ...
        for ctlg in lFtrCols:
            ...
            if ctlg in ['year','runtimes']:
                ...
                oneFtr[0, idxCtlg[lFtrCols.index(ctlg)]] = (float(rst[0]) - mnRnk) / (mxRnk - mnRnk)
            else:
                if rst[0] == 'NULL':
